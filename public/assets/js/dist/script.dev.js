...
$(document).ready(function () {
  ...
  var slider = document.querySelector('.card-listproduct');
  var isDown = false;
  var startX;
  var scrollLeft;
...
function checkLogin() {
  $status = true;
  var id = $('#ch-username').val();
  var pass = $('#ch-password').val();
  $.ajax({
    type: "POST",
    url: URL + "/login/check",
    dataType: "json",
    ...
    if (msg[0] == false) {
      ...
      $('.alert').hide();
      $('#login_error').html(''); // window.location.href = "{{ route('login/do/"+id+"')}}";
    }
  });
  return $status;
}